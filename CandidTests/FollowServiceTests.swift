...
struct FollowServiceTests {
    ...
    private static func makeService() -> (FollowService, TestSupabaseClient.StubbedClient) {
        let stub = TestSupabaseClient.make()
        return (FollowService(client: stub.client, currentUserID: { Self.me }), stub)
    }
    ...
    func followInsertsEdge() async throws {
        ...
        try await service.follow(Self.other)
    ...
    func unfollowDeletesOwnEdge() async throws {
        ...
        try await service.unfollow(Self.other)
    ...
    func isFollowing() async throws {
        ...
        stub.setHandler { _ in .init(body: Self.rowsJSON([(Self.me, Self.other)])) }
    ...
    func relationshipDerivesState() async throws {
        ...
        #expect(stub.requests.count == cases.count * 2)

        // The follows request asks for both directions at once, nothing else.
        let request = try #require(stub.requests.last { $0.url?.path == "/rest/v1/follows" })
    ...
    func relationshipReadsOwnBlock() async throws {
        ...
        let request = try #require(stub.requests.last { $0.url?.path == "/rest/v1/blocks" })
        let query = Self.queryItems(of: request)
    ...
    func blockInsertsRow() async throws {
        ...
        try await service.block(Self.other)
    ...
    func unblockDeletesOwnBlock() async throws {
        ...
        try await service.unblock(Self.other)
    ...
    func relationshipWithSelf() async throws {
        ...
        let relationship = try await service.relationship(with: Self.me)
    ...
    func countsCallTheFunction() async throws {
        let (service, stub) = Self.makeService()
        stub.setHandler { _ in .init(body: Data(#"{"followers":3,"following":1}"#.utf8)) }

        let counts = try await service.counts(for: Self.other)
    ...
    func followLists() async throws {
        ...
        var request = try #require(stub.requests.last)
        #expect(request.httpMethod == "GET")
        #expect(request.url?.path == "/rest/v1/follows")
        ...
        request = try #require(stub.requests.last)
    ...
    func followingCount() async throws {
        ...
        let count = try await service.followingCount()
        #expect(count == 3)
